...
\end{figure}
 ...
 \end{lstlisting}
\section{リミックスツリー}
木構造になっているリミックスツリーは根元のプログラムを元として引用関係を表している。(図\ref{psrt})さらに引用されたプログラムを引用した場合も反映される。(図\ref{rtdit})この図はどのくらい変更が加えられているかがわからない点から元のプログラムと先端のプログラムの類似度を知ることは不可能である。
...
%プログラムをゼロから書くよりもコピーしてパーソナライズできる
%どの程度パーソナライズしたか、追加や変更をしたかを知りたい機会が増える
%その尺度を表す
...
\bibitem{preEssay1}森秀樹・杉澤学・張海・前迫孝憲（2011）「Scratchを用いた小学校プログラミング授業の実践〜小学生を対象としたプログラミング教育の再考〜」\bibitem{preEssay2}小田悠介、若林茂(2013年) 「プログラム間の類似性の定量化手法」,[online]http://www.kobe-kosen.ac.jp/activity/publication/kiyou/Kiyou12/Data/Vol51Paper103-108.pdf
